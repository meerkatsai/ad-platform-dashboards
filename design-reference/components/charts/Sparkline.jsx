...
/* Tiny trend beside a KPI. No axes, no points; ember line, optional dashed compare. */
export function Sparkline({data=[], compare, width=96, height=28, color='var(--series-1)'}){
  const all=[...data,...(compare||[])]; const min=Math.min(...all), max=Math.max(...all), span=(max-min)||1;
  const pts=arr=>arr.map((v,i)=>[(i/(arr.length-1||1))*(width-2)+1, height-2-((v-min)/span)*(height-4)]);
